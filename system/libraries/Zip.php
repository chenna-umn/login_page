...
class CI_Zip
{
    ...
    protected function _add_dir($dir, $file_mtime, $file_mdate)
    {
        ...
        $this->directory .= "PK\x01\x02" . pack("v", $file_mtime) . pack("v", $file_mdate) . pack("V", 0) . pack("V", 0) . pack("V", 0) . pack("v", self::strlen($dir)) . pack("v", 0) . pack("v", 0) . pack("v", 0) . pack("v", 0) . pack("V", 16) . pack("V", $this->offset) . $dir;
    ...
    protected function _add_data($filepath, $data, $file_mtime, $file_mdate)
    {
        ...
        $this->zipdata .= "PK\x03\x04\x14" . pack("v", $file_mtime) . pack("v", $file_mdate) . pack("V", $crc32) . pack("V", $compressed_size) . pack("V", $uncompressed_size) . pack("v", self::strlen($filepath)) . pack("v", 0) . $filepath . $gzdata;
        $this->directory .= "PK\x01\x02" . pack("v", $file_mtime) . pack("v", $file_mdate) . pack("V", $crc32) . pack("V", $compressed_size) . pack("V", $uncompressed_size) . pack("v", self::strlen($filepath)) . pack("v", 0) . pack("v", 0) . pack("v", 0) . pack("v", 0) . pack("V", 32) . pack("V", $this->offset) . $filepath;
        $this->offset = self::strlen($this->zipdata);
        $this->entries++;
        $this->file_num++;
    }
    ...
    public function get_zip()
    {
        ...
        return $this->zipdata . $this->directory . "PK\x05\x06" . pack("v", $this->entries) . pack("v", $this->entries) . pack("V", self::strlen($this->directory)) . pack("V", self::strlen($this->zipdata)) . "";
    }
